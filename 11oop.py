...
# 프로그램을 명령어들의 단순 그룹체라고 보는 시각에서 벗어나 하나의 독립된 객체들의 모음이라고 보는 시각에 근거해서 프로그래밍하는 패러다임
# 프로그램을 보다 유연하게 작성할 수 있고 프로그램 코드의 재사용을 높일 수 있으며 대규모 소프트웨어 개발시 유지보수가 용이해짐
...
class SungJukDAO(): # 처리코드들로 구성된 클래스스
   pass
...
# 객체명만 호출하면 클래스에 정의해둔 __str__가 호출되어 결과 출력됨
print(redCar)

# 성적 처리를 위한 성적SungJuk 클래스를 정의하세요
...
class SungJuk:
    ...
    def __str__(self):
        result = f'{self.name} {self.kor} {self.eng} {self.mat} {self.tot} {self.mean} {self.grd}'
        return result

sj1 = SungJuk('혜교', 86, 54, 76)
print(sj1)

# 사원 관리를 위한 사원Employee 클래스를 정의하세요
# 사원번호, 성, 이름, 이메일, 전화번호, 입사일, 직책, 급여, 수당, 상사번호, 부서번호로 구성
class Employee:
    def __init__(self, empno, lname, fname, email, phone, hdate, job, sal, comm, mgrid, deptid):
        self.empno = empno
        self.lname = lname
        self.fname = fname
        self.email = email
    ...
    def __str__(self):
        result = f'{self.empno} {self.lname} {self.fname} {self.email} {self.phone} {self.hdate} {self.job} {self.sal} {self.comm} {self.mgrid} {self.deptid}'
        return result

emp = Employee('123', 'Smith', 'James', 'sj@abc.com', '02-345-6789', '2019-09-01', 'AD_PRES', 24000, 0, '', 90)
print(emp)